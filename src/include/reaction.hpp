...
public:
                          ...
                          double &phiNew)
        {
                ...
                if (part_id == ELECTRON)
                {
                        phiNew = phi + phiR * gRandom->Gaus(0, 1) * 0.5 * syst_multi_fact;       // * ((-0.4632) * w_val + (2.0038) * w_val + (-0.9035) * w_val); /// 0.4 was the origonal from pass1
                        thetaNew = theta + thetaR * gRandom->Gaus(0, 1) * 0.5 * syst_multi_fact; // * ((-0.4632) * w_val + (2.0038) * w_val + (-0.9035) * w_val);
                        pNew = p + pR * gRandom->Gaus(0, 1) * p * 0.5 * syst_multi_fact;         // * ((-0.4632) * w_val + (2.0038) * w_val + (-0.9035) * w_val);
                }
