...
def file_lookup(input_path, job_number=7535, total=111):
    pattern = f"{job_number}-*.hipo"
    inputs = Path(input_path).rglob(pattern)
    inps = []
    for inp in tqdm(inputs, total=total):
        num = str(inp).split('/')[-1].split("_")[-1].split(".")[0]
        job = inp.parts[-4].split("_")[-1]  # Updated this line
        inps.append({"local": f"{job}_{num}.hipo", "remote": str(inp)})
    if len(inps) < total:
        print(
...
if __name__ == '__main__':
    ...
    jobs = []
    inputs = np.array_split(input_files, 10)
